...
// 표시(목록·메인) 전용 — 상세 페이지 prev/next 등 시간순 소비자에는 적용하지 말 것
export function pinnedFirst<E extends CollectionEntry<"blog"> | CollectionEntry<"projects">>(
  entries: E[]
): E[] {
  return [...entries.filter(isPinned), ...entries.filter((entry) => !isPinned(entry))]
}
